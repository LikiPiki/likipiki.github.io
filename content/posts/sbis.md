...
```vhdl
 B <= "0001";
```
Для каждого сигнала предопределены следующие атрибуты, поддерживаемые программамии синтеза (на примере сигнала s):
- `s'stable[(T)]` --- атрибут равен значению `true` если за время T сигнал не изменял своего состояния.
- `s'transaction` --- атрибут типа `bit` изменяющий состояние на противоположное при каждом новом присваивании нового значения сигналу s.
- `s'event` --- атрибут, равный `true`, если в данном цикле моделирования произошло изменнеие сигнала s.
...
B <= D; --Изменить значение по фронту синхросигнала END IF;
```
...
```vhdl
RESn: if connect_PULLUP=1 generate
...
если целое значение `\connect_PULLUP\` равно 1, то к шине `DATA_BUS` подключаются компоненты нагрузочных резисторов `PULLUP` из библиотеки `UNISIM`.
